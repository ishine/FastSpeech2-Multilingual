...
class VarianceAdaptor(nn.Module):
    ...
    def get_pitch_embedding(self, x, target, mask, control):
        ...
        if target is not None:
            ...
            bins = self.pitch_bins[1] - self.pitch_bins[0]
            if control > 1:
                prediction = prediction + (control - 1) / 0.1 * bins * 10
            else:
                prediction = prediction - (1 - control) / 0.1 * bins * 10
...
class AdversarialClassifier(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, hidden_dims=256, rev_scale=1):
        ...
        # self.lstm = nn.LSTM(input_size=in_dim,hidden_size=hidden_dims,batch_first=True,bidirectional=False)
        # self.linear = nn.Linear(hidden_dims, out_dim)
    ...
    def forward(self, x):
        ...
        # _, (h, c) = self.lstm(x)
        # x = h.transpose(0, 1).reshape(batch_size, -1)
        # x = nn.ReLU()(x)
        # x = self.linear(x)
...
if __name__ == '__main__':
    speaker_classifier = AdversarialClassifier(256, 12, 256, 1)
    input = torch.rand(16, 500, 256)
    output = speaker_classifier(input)
    print(1)
